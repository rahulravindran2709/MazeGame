...
declare module 'three' {
  export class OrbitControls {
    ...
    position0: THREE.Camera['position'];
    zoom0: number;
...
(function () {
  ...
  class OrbitControls extends THREE.EventDispatcher {
    ...
    constructor(object: THREE.Camera, domElement: HTMLElement) {
      ...
      this.zoom0 = (this.object as THREE.PerspectiveCamera).zoom; // the target DOM element for key events
      ...
      this.saveState = function () {
        ...
        scope.zoom0 = (scope.object as THREE.PerspectiveCamera).zoom;
      ...
      this.reset = function () {
        ...
        (scope.object as THREE.PerspectiveCamera).zoom = scope.zoom0;
        (scope.object as THREE.PerspectiveCamera).updateProjectionMatrix();
      ...
      const pan = function () {
        ...
        return function pan(deltaX: number, deltaY: number) {
          ...
          if ((scope.object as THREE.PerspectiveCamera).isPerspectiveCamera) {
            ...
            targetDistance *= Math.tan((scope.object as THREE.PerspectiveCamera).fov / 2 * Math.PI / 180.0); // we use only clientHeight here so aspect ratio does not distort speed
            ...
            // orthographic
            panLeft(deltaX * ((scope.object as THREE.OrthographicCamera).right - (scope.object as THREE.OrthographicCamera).left) / (scope.object as THREE.OrthographicCamera).zoom / element.clientWidth, scope.object.matrix);
            panUp(deltaY * ((scope.object as THREE.OrthographicCamera).top - (scope.object as THREE.OrthographicCamera).bottom) / (scope.object as THREE.OrthographicCamera).zoom / element.clientHeight, scope.object.matrix);
      ...
      function dollyOut(dollyScale: number) {
        ...
        if ((scope.object as THREE.PerspectiveCamera).isPerspectiveCamera) {
          ...
          (scope.object as THREE.OrthographicCamera).zoom = Math.max(scope.minZoom, Math.min(scope.maxZoom, (scope.object as THREE.OrthographicCamera).zoom * dollyScale));
          (scope.object as THREE.OrthographicCamera).updateProjectionMatrix();
          zoomChanged = true;
      ...
      function handleMouseUp(event: MouseEvent) { // no-op
      }
      ...
      function handleTouchEnd(e: TouchEvent) { // no-op
      } //
